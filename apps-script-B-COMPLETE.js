/**
 * ระบบจัดการข้อมูลรายงานแผนกผลิต 2 - กะ B
 ...
 * 
 * หมายเหตุ: กะ B จะบันทึกข้อมูลไปยังคอลัมน์ฝั่ง B (คอลัมน์ฝั่ง A ปล่อยว่าง)
 */

// ===================== CONFIGURATION =====================
const CONFIG = {
  SHEET_ID: '1ZhDdKmzZSK0koExN2u_JsiF_SLAOanYyGtuewNAkFYU',
  FOLDER_ID: '1vGSCN4Qf8XZNewfhpscW4BNZaHV8lvpe',
...
class SheetsManager {
  ...
  /**
   * บันทึกข้อมูลสำหรับกะ B
   * 
   * หมายเหตุ: ฟรอนต์เอนด์ส่งข้อมูลมาในฟิลด์ *A (employeeA, productionA, ฯลฯ)
   * แต่สำหรับกะ B จะบันทึกไปยังคอลัมน์ฝั่ง B และปล่อยคอลัมน์ฝั่ง A ว่างไว้
   * 
   * Mapping:
   * - employeeA -> พนักงานทอ กะ B
   * - productionA -> ยอดทอ กะ B
   * - speedA -> ความเร็วรอบ กะ B
   * - efficiencyA -> ประสิทธิภาพ กะ B
   * - meterStartA -> มิเตอร์เริ่มงาน กะ B
   * - meterEndA -> มิเตอร์เลิกงาน กะ B
   * - lengthA -> ความยาวตัดม้วน กะ B
   */
  saveData(data) {
    ...
    try {
      ...
      const dataMap = (item, index) => ({
        ...
        'พนักงานทอ กะ A': '', // ปล่อยว่าง
        'พนักงานทอ กะ B': item.employeeA || '', // Map จาก employeeA
        'ขนาดหน้าผ้า': item.fabricSize || '',
        'Lot.No': item.lotNo || '',
        'เลขที่ใบสั่งผลิต': item.orderNo || '',
        'ยอดทอ กะ A': '', // ปล่อยว่าง
        'ยอดทอ กะ B': this._parseNumber(item.productionA), // Map จาก productionA
        'ความเร็วรอบ กะ A': '', // ปล่อยว่าง
        'ความเร็วรอบ กะ B': this._parseNumber(item.speedA), // Map จาก speedA
        'ประสิทธิภาพ กะ A': '', // ปล่อยว่าง
        'ประสิทธิภาพ กะ B': this._parseNumber(item.efficiencyA), // Map จาก efficiencyA
        'มิเตอร์เริ่มงาน กะ A': '', // ปล่อยว่าง
        'มิเตอร์เลิกงาน กะ A': '', // ปล่อยว่าง
        'มิเตอร์เริ่มงาน กะ B': this._parseNumber(item.meterStartA), // Map จาก meterStartA
        'มิเตอร์เลิกงาน กะ B': this._parseNumber(item.meterEndA), // Map จาก meterEndA
        'ความยาวตัดม้วน กะ A': '', // ปล่อยว่าง
        'ความยาวตัดม้วน กะ B': this._parseNumber(item.lengthA) // Map จาก lengthA
      });
      ...
      Logger.success(`Successfully saved ${rows.length} rows to Shift B.`);
...
class BackupManager {
  static createBackup(data) {
    ...
    try {
      ...
      const filename = `Backup_ShiftB_${timestamp}.json`;
